...
void modem_initial_setup(void)
{
   ...
   //   err = at_cmd_write("AT%XMAGPIO=1,1,1,7,1,746,803,2,698,748,2,1710,2200,3,824,894,4,880,960,5,791,849,7,1565,1586", modem_at_recv_buf, sizeof(modem_at_recv_buf), &at_state);
...
const char *modem_get_imei(void)
{
   ...
   //   /* IMEI number */
   //   err = at_cmd_write("AT%XICCID", iccid_buf, sizeof(iccid_buf), &at_state);
...
void modem_update_registration_status(void)
{
   ...
   //         enum at_cmd_state at_state;
   //         memset(modem_at_recv_buf, 0, sizeof(modem_at_recv_buf));
   //         at_cmd_write("AT+CGDCONT?", modem_at_recv_buf, sizeof(modem_at_recv_buf), &at_state);
   //         rtc_print_debug_timestamp();
...
void modem_update_information(void)
{
   ...
   //   at_cmd_write("AT", modem_at_recv_buf, sizeof(modem_at_recv_buf), &at_state);
   //   k_msleep(100);
   //   memset(modem_at_recv_buf, 0, sizeof(modem_at_recv_buf));

   //   /* Check connection status */
   //   err = at_cmd_write("AT+CEREG=5", modem_at_recv_buf, sizeof(modem_at_recv_buf), &at_state);

   //   if (!err)
   //   {
   //     err = at_cmd_write("AT+CEREG?", modem_at_recv_buf, sizeof(modem_at_recv_buf), &at_state);
   ...
   //   /* Temperature */
   //   err = at_cmd_write("AT%XTEMP?", modem_at_recv_buf, sizeof(modem_at_recv_buf), &at_state);
...
int16_t get_signal_strength(void)
{
   ...
   //   err = at_cmd_write("AT+CESQ", buf, sizeof(buf), &at_state);
